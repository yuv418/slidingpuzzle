...
use ggez::{
    glam::Vec2,
    graphics::{self, DrawMode, Image, Mesh, Rect},
    Context, GameResult,
};
use keyframe::{functions::EaseInOut, keyframes, AnimationSequence};

use crate::game::{
    animation::DrawablePos, drawable::Drawable as SlidingPuzzleDrawable, resources::theme::Theme, scene::Scene, ui::uitext::UIText,
};
...
impl GameMenuItem {
    ...
    pub fn new_input_item(
        ...
        let prompt_mesh = UIText::new(prompt.to_string(), Theme::bg_color(), 38.0, DrawablePos { x: x + 20.0, y: y + 10.0 });
        Self::new(
            ctx,
            next_page,
    ...
    pub fn new_image_item(
        ctx: &mut Context, image: Arc<Image>, caption: &str, next_page: Option<Box<dyn Fn(&mut Context) -> Box<dyn Scene>>>, x: f32,
        y: f32, w: f32, h: f32,
    ) -> GameResult<Self> {
        ...
        Self::new(
            ...
            GameMenuItemVariant::ImageItem {
                image,
                scale_factor,
                caption_mesh: UIText::new(caption.to_string(), Theme::bg_color(), 28.0, DrawablePos { x: x + 20.0, y: mesh_y }),
            },
            x,
            y,
            w,
    ...
    fn new(
        ...
        Ok(Self {
            ...
            w,
            h,
            item_box_rect: Mesh::new_rounded_rectangle(ctx, DrawMode::fill(), Rect { x: 0.0, y: 0.0, w, h }, 8.0, Theme::fg_color())?,
            border_box_rect: Mesh::new_rounded_rectangle(ctx, DrawMode::fill(), Rect { x: 0.0, y: 0.0, w, h }, 8.0, Theme::border_color())?,
        ...
        match &mut self.item_variant {
            GameMenuItemVariant::TextItem { text_mesh } =>
                for frag in text_mesh.text.fragments_mut() {
                    frag.color = Some(Theme::fg_color());
                },
    ...
    pub fn deselect(&mut self) {
        ...
        match &mut self.item_variant {
            GameMenuItemVariant::TextItem { text_mesh } =>
                for frag in text_mesh.text.fragments_mut() {
                    frag.color = Some(Theme::bg_color());
                },
        ...
        match &mut self.item_variant {
            ...
            GameMenuItemVariant::InputItem { text, prompt_mesh, text_highlight_rect, selected_text_highlight_rect, .. } => {
                ...
                let text_draw = UIText::new(
                    text.to_string(),
                    if self.currently_selected { Theme::bg_color() } else { Theme::fg_color() },
                    28.0,
